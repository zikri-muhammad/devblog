...
<?php
	...
	public function demos_screen() {
		...
		$mt_flag = 0;
		$mt_flag_value = get_option( 'mt_flag_value' );
		if ( empty ( $mt_flag_value ) || 0 == $mt_flag_value ) {
			set_transient( 'news_portal_demo_packages', '', WEEK_IN_SECONDS );
			update_option( 'mt_flag_value', '1' );
		}
...
<?php
	...
	public function news_portal_theme_review_notice() {
		 ...
		 */
		if ( ( get_option( 'news_portal_theme_installed_time' ) > strtotime( '- 15 cal_days_in_month(calendar, month, year)' ) ) || ( $ignored_notice_partially > time() ) || ( $ignored_notice ) ) {
			return;
		}
?>
